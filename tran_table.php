...
$in_query = "SELECT date, transaction_type, amount, description FROM transaction WHERE record_id IN (SELECT record_id FROM financial_record WHERE company_id ='$user_id')";
...
if (!$query) {
    ...
    while ($array = pg_fetch_array($query)) {
        ...
        $data[] = [
            ...
            (float)$array["amount"], // Convert to float for numerical values
            $array["description"]
        ];
    }
}
